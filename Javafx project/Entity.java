...
public abstract class Entity {
    ...
    public void move(double x, double y) {
        ...
        //make a buffer to avoid the situation that entity would be split into two parts on the different sides of the screen
        double buffer = maxWidth / 2;

        // For the positive X-axis
        if (this.component.getTranslateX() > (Constant.WIDTH + buffer)) {
            this.component.setTranslateX((-buffer) + ((this.component.getTranslateX() - (Constant.WIDTH + buffer)) % (Constant.WIDTH + 2 * buffer)));
        }

        // For the negative X-axis
        if (this.component.getTranslateX() < -buffer) {
            this.component.setTranslateX((Constant.WIDTH + buffer) - ((-buffer - this.component.getTranslateX()) % (Constant.WIDTH + 2 * buffer)));
        }

        // For the positive Y-axis
        if (this.component.getTranslateY() > (Constant.HEIGHT + buffer)) {
            this.component.setTranslateY((-buffer) + ((this.component.getTranslateY() - (Constant.HEIGHT + buffer)) % (Constant.HEIGHT + 2 * buffer)));
        }

        // For the negative Y-axis
        if (this.component.getTranslateY() < -buffer) {
            this.component.setTranslateY((Constant.HEIGHT + buffer) - ((-buffer - this.component.getTranslateY()) % (Constant.HEIGHT + 2 * buffer)));
        }
